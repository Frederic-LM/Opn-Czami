...
# --- Local Imports ---
from opn_czami import OpnCzamiLogic, UploadButtonState
from models.config import (
    MAX_LOGO_SIZE_BYTES, MAX_LOGO_PIXELS, STANDARDIZED_LOGO_BASENAME,
    APP_DATA_DIR, MAX_SUMMARY_CHARS, FEATURE_MASKED_IDS
)
from models.utils import show_error, show_info

class CoreWorkflowTabs:
    """Manage 'Issuer Identity' and 'Sign Document' tabs."""

    def __init__(self, notebook: ttk.Notebook, logic: OpnCzamiLogic):
        """Create identity creation/management and document signing tabs."""
    ...
    def _init_state_variables(self):
        ...
        self.url_path_var = tk.StringVar(value="https://")
        self.image_base_url_var = tk.StringVar()
        self.logo_path = None  # Holds the Path object to the selected logo

        # --- Variables for Document Signing ---
        self.include_doc_num_var = tk.BooleanVar(value=False)
        self.auto_gen_doc_num_var = tk.BooleanVar(value=False)
    ...
    def _create_identity_tab(self, parent_frame):
        """Build identity creation form (setup) and management display (manage) frames."""
    ...
    def _create_signer_tab(self, parent_frame):
        """Build document selection, signing, and image display sections."""
        ...
        self.message_text = tk.Text(summary_frame, height=4, wrap="word")
    ...
    def handle_create_identity(self):
        """Validate identity form data and delegate creation to logic layer."""
        ...
        if not url_path or url_path == "https://":
            errors.append("• Public ID URL is required.")
        if not image_base_url or image_base_url == "https://":
            errors.append("• Image Base URL is required.")
        else:
            # Validate that Image Base URL contains a path component
            from urllib.parse import urlparse

            # Normalize URL: if no scheme, add https:// for proper parsing
            test_url = image_base_url
            if not test_url.startswith(('http://', 'https://', 'ftp://')):
                test_url = 'https://' + test_url

            parsed = urlparse(test_url)
            if not parsed.path or parsed.path == '/':
                errors.append("• Image Base URL must include a directory path (e.g., www.mysite.com/lky, not just www.mysite.com)")
    ...
    def handle_delete_identity(self):
        """Confirm and delegate identity deletion to logic layer."""
    ...
    def handle_generate_document_qr(self):
        """Collect signing parameters and start threaded QR/fingerprint generation."""
    ...
    def handle_upload_lkey_file(self):
        """Start manual LKey upload in background thread with progress tracking."""
    ...
    def _run_and_show_upload_result(self, local_path: Path):
        """Upload LKey file and handle success/failure with cleanup."""
        try:
            ...
            if not is_success:
                ...
                try:
                    ...
                    if temp_dir.exists() and temp_dir.is_dir():
                        ...
                        logging.info(f"[UPLOAD] Cleaned up temp directory after successful upload: {temp_dir}")
                except Exception as cleanup_err:
                    logging.warning(f"[UPLOAD] Could not clean up temp directory: {cleanup_err}")

                # DO NOT delete the local .lky file after upload!
                # The file must remain in local storage for dashboard operations (Remove, Delete, etc.)
                # Users can manually delete files if they want to save space.
    ...
    def on_identity_deleted(self):
        """Clear signing tab display after identity deletion."""
        self.clear_lkey_image_display()
        self.message_text.delete("1.0", "end")
        self.qr_display_label.config(image="")

    def on_signing_start(self):
        """Disable signing button and show progress during document signing."""
    ...
    def on_signing_failure(self, message: str):
        """Display error and reset UI after signing failure."""
    ...
    def on_signing_success(self, prepared_upload_path: Path, qr_image_pil: Image.Image, last_signed_payload: str, was_auto_upload_successful: bool, final_lkey_image_with_overlay: Image.Image):
        """Update UI with generated QR, LKey image, and upload status."""
        ...
        if was_auto_upload_successful:
            ...
            # Clean up temp files after successful auto-upload during signing
            try:
                temp_dir = APP_DATA_DIR / "temp_upload"
                if temp_dir.exists() and temp_dir.is_dir():
                    import shutil
                    shutil.rmtree(temp_dir)
                    logging.info(f"[SIGNING] Cleaned up temp directory after successful auto-upload: {temp_dir}")
            except Exception as cleanup_err:
                logging.warning(f"[SIGNING] Could not clean up temp directory: {cleanup_err}")
        else:
    ...
    def update_ui_state(self, has_identity: bool):
        """Switch between identity setup/manage frames and reset signing state."""
        if has_identity:
    ...
    def update_manage_frame_display(self):
        """Populate manage frame with active issuer ID, name, URLs, and contact info."""
    ...
    def update_qr_display(self, pil_image: Image.Image):
        ...
        if not pil_image:
            logging.warning("QR image is None, skipping display update")
            return
    ...
    def _update_doc_num_entry_state(self, *args):
        ...
        is_masking_licensed = self.logic.license_manager.is_feature_enabled(FEATURE_MASKED_IDS)
        ...
        try:
            if parsed := urlparse(self.url_path_var.get()):
                if parsed.scheme and parsed.netloc:
                    new_base_url = f"{parsed.scheme}://{parsed.netloc.rstrip('/')}/lkey/"
                    self.image_base_url_var.set(new_base_url)
